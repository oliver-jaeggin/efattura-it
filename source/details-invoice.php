...
<?php if(isset($_POST['invoice-submit'])): ?>
  ...
  $invoice_discount = addslashes($_POST['invoice-discount']);
  ...
  $invoice_paid = $_POST['invoice-paid'] == 1 ? $_POST['invoice-paid'] : 0;
  ...
  if(!is_numeric($invoice_client_id) && !$invoice_client_id > 0) {
    ...
    $msg_error = 'Hai attivato il campo "Marca di bollo" ma non hai inserito un importo';
  }
  else if($invoice_provision_check == '1' && $invoice_provision == '') {
    $msg_error = 'Hai attivato il campo "Cassa previdenziale (INPS)" ma non hai inserito un\'aliquota';
  }
  else if($invoice_discount_check == '1' && $invoice_discount == '') {
    $msg_error = 'Hai attivato il campo "Sconto" ma non hai inserito un valore';
  }
  else {
    if($item_ids > '') {
    ...
    $sql_update = "UPDATE invoices SET
                  ...
                  inv_client_id='$invoice_client_id',
                  id_items='$item_ids',
                  subtotal='$invoice_subtotal',
                  stamp='$invoice_stamp',
                  provision='$invoice_provision',
                  discount='$invoice_discount',
                  total='$invoice_total',
                  total_rounded='$invoice_total_rounded',
                  exchange_rate='$invoice_exchange_rate',
                  total_eur='$invoice_total_eur',
                  paid='$invoice_paid'
                  WHERE id_inv=$inv_id";
    $res_update = $mysqli->query($sql_update);
    if($res_update){
      $msg_success = 'Aggiornato la fattura con successo';             
    }
    else {
...
<section>
  ...
  <?php
  $sql_inv = "SELECT invoices.*, clients.id_client, clients.cl_display_name
          FROM invoices
          INNER JOIN clients ON invoices.inv_client_id=clients.id_client
          WHERE invoices.id_inv='$inv_id'
  ...
  if($res_inv && $res_inv->num_rows > 0):
    ...
    $call_invoice_id = $inv_id;
    ?>
    <h1><?php echo $title .' no. '. $row_inv['inv_number']; ?></h1>
    <div>
      <?php if(isset($_POST['invoice-submit'])): ?>
      ...
      <?php endif; ?>
        <form id="create-invoice" name="create-invoice" action="details-invoice.php?id=<?php echo $inv_id; ?>" method="POST">
          <div class="field-group flex flex--gap-col-s">
            <div class="field-wrapper flex__grow-2">
              <label for="invoice-client">Cliente</label>
              <input type="text" list="list-clients" name="invoice-client" id="invoice-client" value="<?php echo $row_inv['id_client'] .' - '. $row_inv['cl_display_name']; ?>" autocomplete="off" required>
          ...
          <div id="invoice-base" class="field-group flex flex--gap-col-s">
            <div class="field-wrapper">
              ...
              <input type="text" id="invoice-num" name="invoice-num" value="<?php echo $row_inv['inv_number']; ?>" placeholder="00/YY" require>
            </div>
            <div class="field-wrapper">
              <label for="invoice-date">Data</label>
              <input type="date" id="invoice-date" name="invoice-date" value="<?php echo date('Y-m-d', strtotime($row_inv['inv_date'])); ?>" required>
            </div>
            <div class="field-wrapper">
              <label for="invoice-currency">Divisa</label>
              <div class="flex">
                <input type="text" list="list-currency" id="invoice-currency" name="invoice-currency" minlength="3" maxlength="3" size="6" value="<?php echo $row_inv['inv_currency']; ?>" autocomplete="off" required>
          ...
          <div id="invoice-items" class="sp-y-m">
            ...
            <div class="invoice-items-list sp-y-s">
            ...
            <div class="field-group flex">
              ...
              <div class="field-wrapper">
                <label for="invoice-subtotal">Imponibile [<span class="prefix-currency"><?php echo $row_inv['inv_currency']; ?></span>]</label>
                <span class="input-prefix prefix-currency"><?php echo $row_inv['inv_currency']; ?></span>
                <input type="number" name="invoice-subtotal" id="invoice-subtotal" data-unit="" value="<?php echo $row_inv['subtotal']; ?>" readonly tabindex="-1">
              </div>
            </div>
          </div>
            ...
            <div class="field-group flex flex--gap-col-s">
              <div class="field-wrapper">
                <label for="invoice-discount-check" class="flex flex--pos-x-start sp-y-s">
                  <input type="checkbox" name="invoice-discount-check" id="invoice-discount-check" <?php echo $row_inv['discount'] > '' ? 'checked' : ''; ?> value="1">
                  <p>Sconto</p>
                </label>
              </div>
              <div class="field-wrapper" <?php echo $row_inv['discount'] > '' ? 'data-state="is-opened"' : 'data-state="is-closed"'; ?>>
                <label for="invoice-discount">Sconto (opzionale)</label>
                <span class="input-prefix">&nbsp;&nbsp;%</span>
                <input type="number" name="invoice-discount" id="invoice-discount" data-unit="%" value="<?php echo $row_inv['discount']; ?>">
                <?php
                if($row_inv['discount'] > 0) {
                  $item_subtotal_discount = $row_inv['subtotal'] - ($row_inv['subtotal'] / 100 * $row_inv['discount']);
                }
                else {
                  $item_subtotal_discount = $row_inv['subtotal'];
                }
                ?>
                <p class="text-grey">Importo: <span class="invoice-discount-value"><?php echo $row_inv['subtotal'] > $item_subtotal_discount ? number_format(($row_inv['subtotal'] / 100 * $row_inv['discount']), 2, '.', '') : '0'; ?></span></p>
              </div>
            </div>
            ...
            <div class="field-group flex flex--gap-col-s">
              <div class="field-wrapper">
                <label for="invoice-stamp-check" class="flex flex--pos-x-start sp-y-s">
                  <input type="checkbox" name="invoice-stamp-check" id="invoice-stamp-check" <?php echo $row_inv['stamp'] > '' ? 'checked' : ''; ?> value="1">
                  <p>Marca di bollo</p>
                </label>
              </div>
              <div class="field-wrapper" <?php echo $row_inv['stamp'] > '' ? 'data-state="is-opened"' : 'data-state="is-closed"'; ?>>
                <label for="invoice-provision">Importo bollo [EUR]</label>
                <span class="input-prefix">EUR</span>
                <input type="number" name="invoice-stamp" id="invoice-stamp" data-unit="EUR" value="<?php echo $row_inv['stamp']; ?>">
              </div>
            </div>
            ...
            <div class="field-group flex flex--gap-col-s">
              <div class="field-wrapper">
                <label for="invoice-provision-check" class="flex flex--pos-x-start sp-y-s">
                  <input type="checkbox" name="invoice-provision-check" id="invoice-provision-check" <?php echo $row_inv['provision'] > '' ? 'checked' : ''; ?> value="1">
                  <p>Cassa previdenziale (INPS)</p>
                </label>
              </div>
              <div class="field-wrapper" <?php echo $row_inv['provision'] > '' ? 'data-state="is-opened"' : 'data-state="is-closed"'; ?>>
                <label for="invoice-provision">Aliquota INPS</label>
                <span class="input-prefix">&nbsp;&nbsp;%</span>
                <input type="number" name="invoice-provision" id="invoice-provision" data-unit="%" value="<?php echo $row_inv['provision']; ?>">
                <p class="text-grey">Importo: <span class="invoice-provision-value"><?php echo $item_subtotal_discount ? number_format(($item_subtotal_discount / 100 * $row_inv['provision']), 2, '.', '') : 'Non ancora calcolato'; ?></span></p>
              </div>
            </div>
            <div class="field-group flex flex--gap-col-s flex--pos-y-start">
              <div class="field-wrapper">
                <label for="invoice-total">Importo totale [<span class="prefix-currency"><?php echo $row_inv['inv_currency']; ?></span>]</label>
                <span class="input-prefix prefix-currency"><?php echo $row_inv['inv_currency']; ?></span>
                <input type="number" id="invoice-total" name="invoice-total" data-unit="" value="<?php echo $row_inv['total']; ?>" readonly tabindex="-1">
              </div>
              <div class="field-wrapper">
                <label for="invoice-total-rounded">Importo arrotondato [<span class="prefix-currency"><?php echo $row_inv['inv_currency']; ?></span>]</label>
                <span class="input-prefix prefix-currency"><?php echo $row_inv['inv_currency']; ?></span>
                <input type="number" id="invoice-total-rounded" name="invoice-total-rounded" data-unit="" value="<?php echo $row_inv['total_rounded']; ?>">
              </div>
            </div>
            <div class=" field-groupd flex sp-y-s no-eur-only" data-visible="<?php echo $row_inv['exchange_rate'] > '' ? 'true' : 'false'; ?>">
              <div class="field-wrapper">
                <label for="invoice-exchange-rate">Tasso di cambio <span class="prefix-currency"><?php echo $row_inv['inv_currency']; ?></span> &gt; EUR</label>
                <input type="number" name="invoice-exchange-rate" id="invoice-exchange-rate" step="0.00000001" value="<?php echo $row_inv['exchange_rate']; ?>">
              ...
              <div class="field-wrapper">
                ...
                <span class="input-prefix">EUR</span>
                <input type="number" name="invoice-total-eur" id="invoice-total-eur" data-unit="eur" value="<?php echo $row_inv['total_eur']; ?>" readonly tabindex="-1">
                <p>&nbsp;</p>
              </div>
            </div>
            <div class="field-wrapper sp-y-l">
              <label for="invoice-paid" class="flex flex--pos-x-start sp-y-s">
                <input type="checkbox" name="invoice-paid" id="invoice-paid" <?php echo $row_inv['paid'] == 1 ? 'checked' : ''; ?> value="1">
                <p>Fattura pagato</p>
              </label>
            </div>
            <div class="field-group flex flex--pos-x-start flex--gap-col-m">
              <button type="submit" name="invoice-submit" value="invoice-submit">Salva</button>
              <a href="index.php" class="btn btn--sec-color">Ritorna alla bacheca</a>
              <button type="button" class="btn btn--icon-text" onclick="globalFun.calcInvTotals(<?php echo $row_inv['subtotal']; echo $row_inv['discount'] > 0 ? ', '. $row_inv['discount'] : ', 0'; echo $row_inv['provision'] > 0 ? ', '. $row_inv['provision'] : ', 0'; ?>);">
    ...
    <?php else: ?>
      ...
      <div class="msg">
        <p class="sp-y-m">Non stato trovato la fattura richiesta!</p>
